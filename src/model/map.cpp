...
std::vector<Fighter *> Map::getFighter(const TypeOfFighter &type, int player) const {
  std::vector<Fighter *> result;
  for (auto &[tileId, fighter] : occupancy) {
    if (fighter == nullptr) continue;
    if (fighter->getOwnerPlayer() != player) continue;
    if (fighter->getFighterType() != type) continue;
    result.push_back(fighter);
  }
  return result;
}
// tiles that can reach with a value(distance)
std::vector<Tile *> Map::getTilesThatCanMoveTo(int fromTileId, int distance) const {
  std::vector<Tile *> result;
  if (!getTile(fromTileId)) return result;

  std::unordered_map<int, int> depth;
  std::queue<int> frontier; // using queue for bfs
  depth[fromTileId] = 0;
  frontier.push(fromTileId);

  while (!frontier.empty()) {
    int current = frontier.front();
    frontier.pop();
    int currentDepth = depth[current];
    if (currentDepth >= distance) continue;

    Tile *tile = getTile(current);
    if (!tile) continue;

    for (int neighborId : tile->getNeighbors()) {
      if (depth.find(neighborId) != depth.end()) continue;
...
int Map::distanceBetween(int tileIdA, int tileIdB) const {
  if (tileIdA == tileIdB) return 0;
  if (!getTile(tileIdA) || !getTile(tileIdB)) return -1;

  std::unordered_map<int, int> depth;
  std::queue<int> frontier;
  depth[tileIdA] = 0;
  frontier.push(tileIdA);

  while (!frontier.empty()) {
    int current = frontier.front();
    frontier.pop();
    if (current == tileIdB) return depth[current];
...
// same zone get
std::vector<Tile *> Map::getTilesInZones(const std::unordered_set<std::string> &zones) const {
  std::vector<Tile *> result;
...
std::vector<int> Map::getPortalTileIds() const {
  ...
  for (auto &[id, tile] : tiles) {
    if (tile->isPortal()) codes.push_back(id);
  }
  return codes;
}
// not occuputed by others and...
std::vector<Tile *> Map::getReachableTiles(int fromTileId, int distance, const Fighter *mover) const {
  std::vector<Tile *> result;
  if (!getTile(fromTileId)) return result;
  ...
  while (!frontier.empty()) {
    ...
    if (current != fromTileId) {
      ...
      if (fighter && fighter->getOwnerPlayer() != moverOwner) continue;
    }
    ...
    if (tile->isPortal()) {
      for (int portalId : getPortalTileIds()) {
        if (portalId == current) continue;
        if (depth.find(portalId) != depth.end()) continue;
        depth[portalId] = currentDepth + 1;
        frontier.push(portalId);
      }
    }
  }
...
int Map::movementDistance(int fromTileId, int toTileId, const Fighter *mover) const {
  if (fromTileId == toTileId) return 0;
  if (!getTile(fromTileId) || !getTile(toTileId)) return -1;
  ...
  while (!frontier.empty()) {
    int current = frontier.front();
    frontier.pop();
    if (current == toTileId) return depth[current];

   
    if (current != fromTileId) {
      Fighter *occupant = getFighterAt(current);
      if (occupant && occupant->getOwnerPlayer() != moverOwner) continue;
    }
