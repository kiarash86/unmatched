...
TypeOfCard Card::getCardType() const
{
    return type;
}
TypeOfEvent Card::getEventType() const
{
    return eventType;
}
    std::vector<std::unique_ptr<Effect>> & Card::getEffects()
{
    return effects;
}
std::string Card::getImgSource() const
{   
    return imgSource;
}
TypeOfPerformer Card::getPerformer() const
{
    return performer;
}
void Card::setBoost(const int & newBoost)
{
    boost =newBoost;
}
...
void Card::setCardType(const TypeOfCard &newType)
{
    type =newType;
}
